...
WebConfig::WebConfig() {
    ...
    macID.toUpperCase();
    String AP_NameString = WIFI_SSID_PREFIX + macID;
    ...
    server = new ESP8266WebServer(WEB_CONFIG_PORT);

    server->on("/", this->handleRoot);
    server->on("/resetEEPROM", this->handleResetEEPROM);
    server->on("/saveParams", this->handleSaveParameters);
    server->on("/css", this->handleCSS);
...
void WebConfig::handleRoot() {
    ...
    itoa(storage->configuration.robotId, tmp_str, 10);
    content.replace("$ROBOT_ID", tmp_str);

    content.replace("$CONFIG_VERSION", storage->CONFIG_VERSION);
    content.replace("$FIRMWARE_VERSION", FIRMWARE_VERSION);
    content.replace("$HARDWARE_VERSION", HARDWARE_VERSION);

    content.replace("$ROBOT_DESC", storage->configuration.robotDesc);
    content.replace("$WIFIC_SSID", storage->configuration.wifiSSID);
    content.replace("$WIFIC_PASSWD", storage->configuration.wifiPass);
    content.replace("$ROS_MASTER", storage->configuration.rosMasterAddress);

    server->send(200, "text/html", content);
    Serial.println("content sent");
}

void WebConfig::handleSaveParameters() {
    Serial.println("handleSaveParams()");
    String robotId = server->arg("robotId");
    String robotDesc = server->arg("robotDescription");
    String wifiSSID = server->arg("wificSSID");
    String wifiPwd = server->arg("wificPassword");
    String rosMaster = server->arg("rosMasterAddress");

    storage->configuration.robotId = robotId.toInt();
    strcpy(storage->configuration.robotDesc, robotDesc.c_str());
    strcpy(storage->configuration.wifiSSID, wifiSSID.c_str());
    strcpy(storage->configuration.wifiPass, wifiPwd.c_str());
    strcpy(storage->configuration.rosMasterAddress, rosMaster.c_str());
    storage->configuration.isAPMode = false;
...
void WebConfig::handleResetEEPROM() {
    char tmp_str[20] = "";

    storage->configuration.robotId = -1;
    strcpy(storage->configuration.robotDesc, tmp_str);
    strcpy(storage->configuration.rosMasterAddress, tmp_str);
    strcpy(storage->configuration.wifiSSID, tmp_str);
    strcpy(storage->configuration.wifiPass, tmp_str);
    storage->configuration.isAPMode = true;
